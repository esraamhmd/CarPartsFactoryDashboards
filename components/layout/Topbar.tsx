...
function useIsMobile() { const [m,setM]=_us(false); _ue(()=>{const fn=()=>setM(window.innerWidth<640);fn();window.addEventListener('resize',fn);return()=>window.removeEventListener('resize',fn);},[]);return m; }
...
export default function Topbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <div className="topbar">
      {/* Hamburger */}
      <button aria-label="action" onClick={onMenuClick} className="menu-btn" style={{ ...btnStyle, display:'none', flexShrink:0 }}>
      ...
      <div ref={sRef} style={{ flex:1, maxWidth:480, position:'relative' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, background:'var(--bg-input)', borderWidth:'1px',borderStyle:'solid',borderColor:'var(--border)', borderRadius:10, padding:'8px 13px' }}
          ...
          {search && <button aria-label="action" onClick={()=>{setSearch('');setShowSearch(false);}} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', display:'flex', padding:0 }}><MdClose size={14}/></button>}
        </div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:6, marginInlineStart:'auto' }}>
        <button onClick={()=>setLang(lang==='en'?'ar':'en')} aria-label="Toggle language" style={btnStyle}>
          <MdLanguage size={15}/><span style={{ fontFamily: isRTL ? "Tahoma,Arial,sans-serif" : "inherit" }}>{lang==='en'?'عربي':'EN'}</span>
        </button>
        <button onClick={toggleTheme} aria-label="Toggle theme" style={btnStyle}>
          {theme==='light'?<MdDarkMode size={17}/>:<MdLightMode size={17}/>}
        </button>

        {/* Notifications */}
        <div ref={nRef} style={{ position:'relative' }}>
          <button onClick={()=>{setShowNotifs(p=>!p);setShowUser(false);}} aria-label="Notifications" style={{ ...btnStyle, position:'relative' }}>
          ...
          {showNotifs && (
            <div style={{ ...(isMobile ? {position:'fixed' as const,top:70,left:8,right:8,width:'auto'} : {position:'absolute' as const,top:46,insetInlineEnd:0,width:360}), background:'var(--bg-card)', borderWidth:'1px', borderStyle:'solid', borderColor:'var(--border)', borderRadius:14, maxHeight:'70vh', boxShadow:'0 20px 60px rgba(0,0,0,0.20)', zIndex:9999, overflow:'hidden', display:'flex', flexDirection:'column' }}>
