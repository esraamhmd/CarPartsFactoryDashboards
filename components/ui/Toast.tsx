        ...
        {toasts.map(t => {
          ...
          return (
            <div key={t.id} style={{
              ...
              <button aria-label="action" onClick={() => remove(t.id)} style={{ background: 'none', border:'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', padding: 2, flexShrink: 0, pointerEvents: 'all' }}>
                <MdClose size={15} />
              </button>
            </div>
          );
